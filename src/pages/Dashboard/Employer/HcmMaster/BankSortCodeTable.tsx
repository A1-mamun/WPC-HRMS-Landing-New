...
const BankSortCodeTable = () => {
  ...
  const [createSortCode] = useCreateOrganisationBankSortCodeMutation();
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await createSortCode({ data: { name } }).unwrap();
      toast.success("Sort Code created successfully!", {
        id: toastId,
        duration: 2000,
      });
      onOpenChange();
      ...
      toast.error("Failed to create sort code.", {
        id: toastId,
        duration: 3000,
      });
    }
