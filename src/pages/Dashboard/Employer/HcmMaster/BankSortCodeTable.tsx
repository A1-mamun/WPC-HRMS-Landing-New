...
import { useGetOrganisationBankSortCodesQuery } from "../../../../redux/features/employer/hcmMaster";
import { Spinner } from "@heroui/react";

const BankSortCodeTable = () => {
  const { data, isLoading, isError, refetch, isFetching } =
    useGetOrganisationBankSortCodesQuery(undefined);
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[222px]">
        <Spinner
          ...
          aria-label="Loading bank sort codes"
        />
        <p className="text-gray-600 mt-4 text-base">
          Loading bank sort codes...
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[222px]">
        <p className="text-red-600 font-semibold text-xl">
          Failed to load bank sort codes
        </p>
        <button
          onClick={refetch}
  ...
  if (data?.data?.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[222px]">
        <p className="text-gray-600 text-lg">No bank sort codes found</p>
      </div>
    );
  }
...
export default BankSortCodeTable;
