...
const TaxMasterTable = () => {
  const { data, isLoading, isError, refetch, isFetching } =
    useGetOrganisationTaxMastersQuery(undefined);
  const { isOpen, onOpen, onOpenChange } = useDisclosure();

  const [createTaxMaster] = useCreateOrganisationTaxMasterMutation();
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const toastId = toast.loading("Creating Tax Master...");

    try {
      await createTaxMaster({ data: { name } }).unwrap();
