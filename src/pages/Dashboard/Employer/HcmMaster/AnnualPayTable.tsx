...
const AnnualPayTable = () => {
  ...
  const [createAnnualPay] = useCreateOrganisationAnnualPayMutation();
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await createAnnualPay({ data: { name } }).unwrap();
      toast.success("Annual Pay created successfully!", {
        id: toastId,
        duration: 2000,
      });
      onOpenChange();
      ...
      toast.error("Failed to create annual pay.", {
        id: toastId,
        duration: 3000,
      });
    }
